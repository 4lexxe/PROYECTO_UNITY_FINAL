...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (attackDown && isGrounded && !attackLock && (Time.time - lastAttackTime) >= attackCooldown)
        {
            attackPressed = true;
    ...
    void FixedUpdate()
    {
        ...
        if (attackLock && (Time.time - lastAttackTime) >= attackDuration)
        {
            attackLock = false;
        }
...
reTag(groundTag)) { isGrounded = true; break; }
                }
            }
        }
