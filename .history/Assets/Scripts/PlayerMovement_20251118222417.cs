...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (attackDown && !attackLock)
        {
            ...
            {
                anim.CrossFadeInFixedTime(attackStateName, 0f);
            }
        }
        ...
        var stU = anim.GetCurrentAnimatorStateInfo(0);
        if (isAttacking && (stU.IsName(attackStateName) && stU.normalizedTime >= 1f))
        {
            isAttacking = false;
            attackLock = false;
            anim.speed = 1f;
