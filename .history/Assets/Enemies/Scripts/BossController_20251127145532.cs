    ...
    GameObject CreateSimpleProjectile(Vector3 origin, Vector2 dir)
    {
        ...
        public class ProjectileDamage2D : MonoBehaviour
        {
            ...
            void OnTriggerEnter2D(Collider2D other)
            {
                ...
                var pmSelf = other.GetComponent<PlayerMovement>();
                if (pc == null) pc = other.GetComponentInParent<PlayerController>();
                if (pmSelf == null) pmSelf = other.GetComponentInParent<PlayerMovement>();
                bool isPlayer = other.CompareTag(targetTag) || pc != null || pmSelf != null;
                if (isPlayer)
                {
                    if (pmSelf != null && pmSelf.IsAttackingForProjectile())
                    {
                        Break();
                        return;
                    }
                    ...
                    {
                        pmSelf.gameObject.SendMessage("RecibirDanio", damage, SendMessageOptions.DontRequireReceiver);
                    }
                    Destroy(gameObject);
                    return;
                }
            ...
            void OnTriggerStay2D(Collider2D other)
            {
                ...
                if (pmSelf == null) pmSelf = other.GetComponentInParent<PlayerMovement>();
                bool isPlayer = other.CompareTag(targetTag) || pc != null || pmSelf != null;
                if (isPlayer && pmSelf != null && pmSelf.IsAttackingForProjectile())
                {
                    Break();
                }
            }
    ...
    bool HasParam(string param, AnimatorControllerParameterType type)
    {
        ...
        return false;
            ...
            void OnCollisionStay2D(Collision2D collision)
            {
                ...
                if (pm != null && pm.IsAttackingForProjectile()) Break();
            }
