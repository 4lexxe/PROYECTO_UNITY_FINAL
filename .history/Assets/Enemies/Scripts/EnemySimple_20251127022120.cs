    ...
    GameObject CreateSimpleProjectile(Vector3 origin, Vector2 dir)
    {
        ...
        public class ProjectileDamage2D : MonoBehaviour
        {
            ...
            void OnTriggerEnter2D(Collider2D other)
            {
                ...
                var pc2 = other.GetComponentInParent<PlayerController>();
                if (pc2 != null)
                {
                    var pa2 = pc2.GetComponentInChildren<Animator>();
                    if (pa2 != null)
                    {
                        var st2 = pa2.GetCurrentAnimatorStateInfo(0);
                        bool playerIsAttacking2 = st2.IsName("attack_0") || st2.IsName("attack_1");
                        if (playerIsAttacking2)
                        {
                            Break();
                            return;
                        }
