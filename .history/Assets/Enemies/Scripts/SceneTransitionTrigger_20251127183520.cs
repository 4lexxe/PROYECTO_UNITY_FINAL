...
public class SceneTransitionTrigger : MonoBehaviour
{
    ...
    void CreatePortalFX()
    {
        ...
        for (int i = portalRoot.childCount - 1; i >= 0; i--)
        {
            ...
            if (c != null && c.name.StartsWith("Orb")) DestroyImmediate(c.gameObject);
        }
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (mr != null) mr.material.color = portalColor;
            var sc = sphere.GetComponent<SphereCollider>();
            if (sc != null) DestroyImmediate(sc);
        ...
        if (!showPortalText)
        {
            if (textRoot != null)
            {
                DestroyImmediate(textRoot.gameObject);
                textRoot = null;
            }
        }
        else
